...
export default function AreasPage() {
  return (
    <div className="mx-auto max-w-6xl px-5 pb-16 pt-[clamp(6rem,10vw,8.5rem)] md:px-8">
      ...
      <ul className="mt-5 flex flex-wrap gap-x-6 gap-y-3">
        {cityPages.map((c) => (
          <li key={c.slug}>
            <Link
              href={`/areas/${c.slug}`}
              className="tap font-semibold text-green underline decoration-2 underline-offset-4"
            >
              {c.name}
            </Link>
          </li>
      ...
      <div className="mt-8 grid gap-x-10 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
        {bayAreaCounties.map((county) => (
          <section key={county.county}>
            <h3 className="h3 border-b border-line pb-2">{county.county}</h3>
                ...
                return (
                  <li key={city}>
                    {slug ? (
                      <Link
                        href={`/areas/${slug}`}
                        className="tap font-medium text-green underline underline-offset-2"
                      >
                        {city}
                      </Link>
                    ) : (
      ...
      <p className="mt-12 max-w-[60ch] text-[0.95rem] text-mute-l">
